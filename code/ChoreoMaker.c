...
static float lines1[NUM_OF_LINES][2],lines2[NUM_OF_LINES][2],lines3[NUM_OF_LINES][2],lines4[NUM_OF_LINES][2], /*Matrice u kojima se cuvaju koordinate igraca*/
             lines5[NUM_OF_LINES][2],lines6[NUM_OF_LINES][2],lines7[NUM_OF_LINES][2],lines8[NUM_OF_LINES][2];


static void onDisplay(void);
static void onTimer1(int value); /* Funkcije za pomeranje za svakog igraca*/
...
static void onDisplay(void)
{
    ...
    /*Postavljanje svetla za lopte (igrace)*/
...
static void onTimer1(int value) 
{
    ...
    if(a1>0)
    {
        if(x1_f >= x1_t)
        {
            ...
            /*Pocetna tacka dobija vrednost trenutne tacke, trenutnih koordinata*//*Sledaca na koju treba da stigne se uzima iz matrice u kojoj se cuvaju koordinate igraca*/
            x1_f=x1_t;
            z1_f=z1_t;
            
            /*Sledaca na koju treba da stigne se uzima iz matrice u kojoj se cuvaju koordinate igraca*/
...
/*Sce onTimer funkcije isto funkcionisu, samo su u pitanju drugi igraci*/
static void onTimer2(int value)
{
    if(value!=2)
        return;
...
static void onKeyboard(unsigned char key, int x, int y)
{
    ...
    case 'S':
        /*Otvara se falj u oji se ispisuju zabelezene koordinate igraca, tj linije*/
        fptr=fopen("lines.txt","w");
        if(fptr==NULL)
            printf("problem sa otvaranjem fajla\n");
        /*U jednom redu se cuvaju po jedan par koordinata za svakog igraca */
        for(i=0; i<=line; i++)
    ...
    case 'b':
        /*U zavisnosti od toga koji igrac je odabran pomera se za pola metra unazad (u realnosti)*/
        if(id==1)
            z1_f-=0.05;
        if(id==2)
            z2_f-=0.05;
    ...
    /*U zavisnosti od toga koji igrac je odabran pomera se za pola metra unapred (u realnosti)*/    
    case 'f':
        if(id==1)
            z1_f+=0.05;
        if(id==2)
    ...
    /*U zavisnosti od toga koji igrac je odabran pomera se za pola metra ulevo (u realnosti)*/
    case 'l':
        if(id==1)
            x1_f-=0.05;
        if(id==2)
    ...
    /*U zavisnosti od toga koji igrac je odabran pomera se za pola metra udesno (u realnosti)*/
    case 'r':
        if(id==1)
            x1_f+=0.05;
        if(id==2)
    ...
    case 'n':
        ...
        /*Cuvaju se trenutne pozicije lopti (igraca)*/
        line+=1;
        lines1[line][0]=x1_f;
        lines1[line][1]=z1_f;
        lines2[line][0]=x2_f;
